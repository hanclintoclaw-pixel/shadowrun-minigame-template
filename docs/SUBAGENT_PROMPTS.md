...
- The issue body must include a human summary and a fenced JSON block with `schemaVersion: "shadowrun-minigame-persist/v1"` and JSON Patch delta operations when the payload fits in a safe URL; otherwise download the JSON request and open a short issue instructing the user to attach it.
